...
def estimate(counts, left, right, out=True, verbose=False):
    ...
    MLlambda = ret
    libsize = 1.0 * xcount / K(MLlambda, left, right)
    libsize2 = 1.0 * xcount / max(1e-12, K(ret2, left, right, cdf=scipy.stats.nbinom.cdf))
    libsize3 = 1.0 * xcount / K(ret3, left, right, cdf=scipy.stats.logser.cdf)
...
def estimate_EM(counts, left, right):
    ...
    for its in xrange(20):
        ...
        unobs = (1.0 / K(L, left, right) - 1.0) * xcount
...
if __name__ == "__main__":
    ...
    if args.subsample < 1.0 and args.subsample > 0.0:
        ...
        sys.stderr.write("randomly downsampling to %.4f of reads\n" % args.subsample)
    else:
        subsample = counts
